...
def _emit_message(
    ...
    Each field entry uses the pattern ``name : [qualifiers] type (#tag)``
    ...
    if msg.field:
        ...
        for field_idx, field in enumerate(msg.field):
            ...
            # Term: ``field_name`` : [qualifiers] type (#tag)
            out.append(f"``{field.name}`` : {prefix}{type_repr} (#{field.number})")
