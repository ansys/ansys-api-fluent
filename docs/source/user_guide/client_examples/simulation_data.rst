...
.. testcode::
   ...
   assert data_availability_response.is_data_available

   boundary_values_response = field_data_stub.IsBoundaryValuesEnabled(
       field_data_pb2.IsBoundaryValuesEnabledRequest(),
       metadata=metadata,
   )
   assert isinstance(boundary_values_response.is_boundary_values_enabled, bool)
...
.. doctest::
   ...
   symmetry-xyplane: surface_ids=[5]
   hot-inlet: surface_ids=[4]
   cold-inlet: surface_ids=[3]
   outlet: surface_ids=[2]
   wall-inlet: surface_ids=[1]
   wall-elbow: surface_ids=[0]
...
.. testcode::
   ...
   assert {'pressure', 'temperature', 'velocity-magnitude'} <= set(scalar_fields)
   ...
   assert {'velocity', 'relative-velocity'} <= set(vector_fields)
...
.. doctest::
   ...
   ...     metadata=metadata,
   ... )
   >>> round(range_response.minimum, 0)
   -0.0
   >>> round(range_response.maximum, 0)
   1.0
...
.. testcode::
   ...
   assert byte_stream is not None
   byte_stream.cancel()
...
.. doctest::
   ...
   ...     metadata=metadata,
   ... )
   >>> print(area.value.double_state > 0)
   True
...
.. testcode::
   ...
   assert count.value.double_state >= 0
   ...
   assert count_if.value.double_state >= 0
   assert count_if.value.double_state <= count.value.double_state
   ...
   assert min_result.value.double_state <= max_result.value.double_state
...
.. testcode::
   ...
   assert area_ave.value.double_state > 0
   ...
   assert area_int.value.double_state > 0
...
.. doctest::
   ...
   ...     metadata=metadata,
   ... )
   >>> print(round(mf_ave.value.double_state, 1))
   101318.1
   ...
   ...     metadata=metadata,
   ... )
   >>> print(round(mf_ave_abs.value.double_state, 1))
   101325.2
   ...
   ...     metadata=metadata,
   ... )
   >>> print(round(mf_int.value.double_state, 4))  # Pa·kg/s
   -35.1649
...
.. testcode::
   ...
   assert (min_result.value.double_state
         <= vol_ave.value.double_state
         <= max_result.value.double_state)
   ...
   assert vol_int.value.double_state > 0
...
.. testcode::
   ...
   assert mass_ave.value.double_state > 0
   ...
   assert mass_int.value.double_state > 0
...
.. doctest::
   ...
   ...     metadata=metadata,
   ... ).value
   >>> print(f"pressure force: ({pressure_force.x:.4g}, {pressure_force.y:.4g}, {pressure_force.z:.4g})")  # doctest: +SKIP
   pressure force: (-0.0231, 0.0, 1.452)
   >>> print(f"viscous force: ({viscous_force.x:.4g}, {viscous_force.y:.4g}, {viscous_force.z:.4g})")  # doctest: +SKIP
   viscous force: (-0.0012, 0.0, 0.0034)
   >>> print(f"total force: ({total_force.x:.4g}, {total_force.y:.4g}, {total_force.z:.4g})")  # doctest: +SKIP
   ...
   >>> print(f"moment: ({moment_coords.x:.4g}, {moment_coords.y:.4g}, {moment_coords.z:.4g})")  # doctest: +SKIP
...
.. testcode::
   ...
   assert total.value.double_state > 0
   ...
   assert conditional.value.double_state >= 0
   assert conditional.value.double_state <= total.value.double_state
...
.. doctest::
   ...
   ...     print(f"zone {z.zone_id}: {z.name}  type={solution_variable_pb2.ThreadType.Name(z.thread_type)}")
   zone 29: symmetry-xyplane  type=THREAD_TYPE_FACE
   zone 30: hot-inlet  type=THREAD_TYPE_FACE
   zone 31: cold-inlet  type=THREAD_TYPE_FACE
   zone 32: outlet  type=THREAD_TYPE_FACE
   zone 33: wall-inlet  type=THREAD_TYPE_FACE
   zone 34: wall-elbow  type=THREAD_TYPE_FACE
   zone 87: elbow-fluid  type=THREAD_TYPE_CELL
   zone 89: interior--elbow-fluid  type=THREAD_TYPE_FACE
   ...
   >>> assert "SV_P" in names
   >>> print(sorted(names, key=len)[:4])
   ['SV_H', 'SV_K', 'SV_O', 'SV_P']
...
.. doctest::
   ...
   ...               f"{chunk.payload_info.field_size} cells")  # doctest: +ELLIPSIS
   ...     elif part == "payload":
...
.. testcode::
   ...
   assert isinstance(set_svar_response, solution_variable_pb2.SetSolutionVariableDataResponse)
