...
.. doctest::
   ...
   ...     metadata=metadata,
   ... )
   >>> print(schema_response.info.type)
   group
   >>> print(len(schema_response.info.children))  # number of top-level nodes
   10
   ...
   >>> walk(schema_response.info)  # doctest: +SKIP
...
.. doctest::
   ...
   ...     metadata=metadata,
   ... )
   >>> print(round(value_to_python(state_response.value), 0))
   101325
   ...
   ...     metadata=metadata,
   ... )
   >>> energy_map = value_to_python(state_response.value)
   >>> print(dict(sorted(energy_map.items())))
   {'enabled': True, ..., 'viscous-dissipation': False}
...
.. doctest::
   ...
   ...     metadata=metadata,
   ... )
   >>> print(value_to_python(check.value))
   101000
...
.. doctest::
   ...
   >>> for attr, val in sorted(attrs_response.values.value_map.m.items(), key=lambda item: item[0]):
...
.. doctest::
   ...
   ...     metadata=metadata,
   ... )
   >>> # names of existing wall BCs
   >>> print(sorted(objects.names))  # names of existing wall BCs
   ['wall-elbow', 'wall-inlet']
       ...
       settings_pb2.ExecuteCommandRequest(
           ...
           args=settings_pb2.Value(
               value_map=settings_pb2.Value.ValueMap(
                   m={"number-of-iterations": settings_pb2.Value(integer=1)}
               )
           ),
       ),
       metadata=metadata,
...
.. doctest::
   ...
   ...     metadata=metadata,
   ... )
   >>> print(value_to_python(query_response.reply))  # could also be None
   Scalar
...
.. doctest::
   ...
   >>> for token in ("*", "?", "cold-inlet", ""):  # ? may not be a wildcard depending on Fluent version
